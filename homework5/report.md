...
##解題說明
本題要求實作一個Graph抽象基底類別，並利用鄰接鏈結串列來儲存有向圖，並且實作AOV和AOE關鍵路徑演算法。

##解題策略
1. 由於 Graph 類別的頂點數 $n$ 與邊數 $e$ 被宣告為 $private$ ，所以透過 $setN()$ 和 $addE()$ 來更新圖形。
2. 計算關鍵路徑時，先透過 $TpSort$ 取得順序並儲存於 $sE$ 。接著計算各事件的最早發生時間 $ee$，再計算最晚發生時間 $le$。
...
| 測試案例 | 輸入參數  | 實際輸出 |
|----------|---------------|----------|
| 測試一   |<img width="316" height="191" alt="image" src="https://github.com/user-attachments/assets/6384779d-4590-40c8-914f-cedd59bafd0f" />|<img width="200" height="81" alt="image" src="https://github.com/user-attachments/assets/9a123185-4b30-42c0-8df2-efcb0ab266c8" />| 
| 測試二   | <img width="253" height="115" alt="image" src="https://github.com/user-attachments/assets/2fc6a105-1f88-4e39-8e95-4be74b38a606" />|<img width="213" height="33" alt="image" src="https://github.com/user-attachments/assets/bd996da1-1cdd-4cbb-b3b1-c0befb4ad40b" />|

###結論
1. 程式能以鄰接鏈結串列正確表示有向圖。
2. 透過事件最早發生時間 $ee$ 與最晚發生時間 $le$ 的雙向推導，能夠正確判別出活動最早開始時間 $Ee$ 與最晚開始時間 $Le$ 相等的關鍵活動。

##申論及開發報告
父類別 Graph 所規範的 InsertEdge(u, v) 純虛擬函式並未提供權重輸入，所以在後面另外新增一個InsertEdgeW(u, v, w)完成銓重輸入。
